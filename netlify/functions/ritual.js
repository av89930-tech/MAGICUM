...
exports.handler = async (event) => {
  ...
  try {
    ...
    const prompt = `You are a professional furniture reupholstery visualizer. Task: take the FIRST image (furniture) and reupholster it using the fabric from the SECOND image. Rules: 1) Keep EXACTLY the same camera angle, composition, perspective, zoom level, and background as the first image. Do NOT zoom in. Do NOT crop any edges of the furniture — show the COMPLETE furniture exactly as framed in the first image. 2) Keep the furniture shape, frame, legs, cushion forms, shadows and room environment identical. 3) Replace ONLY the upholstery/fabric surface with the texture and color pattern from the second image. 4) COLOR ACCURACY: match the fabric color 1:1 — zero deviation in hue, saturation, or brightness. Use the second image as the absolute color reference. Render accurate color in both shadows and highlights under natural daylight (5500K white balance). 5) The result must look photorealistic. 6) Output a SINGLE image showing ONLY the reupholstered furniture. Do NOT include fabric swatches, color samples, comparison panels, labels, watermarks, or any elements other than the furniture itself.`;
